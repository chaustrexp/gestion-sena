...
4. **Reparar codificación UTF-8** (IMPORTANTE)
   - Ejecutar: `REPARAR_UTF8.bat` (doble clic)
   - O abrir en navegador: `http://localhost/Gestion-sena/dashboard_sena/REPARAR_UTF8_AHORA.php`
   - Esto corrige caracteres como: Configuración, Tecnología, etc.
   - Presionar Ctrl+F5 después para limpiar caché
...
- **Caracteres raros (Ã, Â, etc.)**: Ejecutar `REPARAR_UTF8.bat` o el script PHP de reparación

## Problema de Codificación UTF-8

Si ves caracteres como:
- ❌ `Configuración` en lugar de `Configuración`
- ❌ `TecnologÃ­a` en lugar de `Tecnología`
- ❌ `GestiÃ³n` en lugar de `Gestión`

**Solución rápida:**

1. **Opción 1 (Recomendada)**: Ejecutar `REPARAR_UTF8.bat`
2. **Opción 2**: Abrir en navegador `http://localhost/Gestion-sena/dashboard_sena/REPARAR_UTF8_AHORA.php`
3. Presionar `Ctrl+F5` en el dashboard para limpiar caché

Ver documentación completa en: `SOLUCION_UTF8_RAPIDA.md`
